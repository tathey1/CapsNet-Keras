'''
Borrows code from XifengGuo/CapsNet-Keras/capsulenet.py in order to implement
network from:
Convolutional capsule network for classification of breast cancer histology images
Iesmantas 2017

Thomas Athey 8/21/2018
'''
...
def CapsNet(input_shape, n_class, routings):
    ...
    conv2 = layers.Conv2D(filters=128, kernel_size=4, strides=2, padding='valid',
                          activation='relu', name='conv2')(conv1)
    conv3 = layers.Conv2D(filters=256, kernel_size=6, strides=2, padding='valid',
...
def train(model, data, args):
    ...
    tb = callbacks.TensorBoard(log_dir=args.save_dir + '/tensorboard-logs',
                               batch_size=args.batch_size_train, histogram_freq=int(args.debug))
    ...
    def list_generator(x, y, batch_size, shift_fraction=0.):
    ...
    train_gen = list_generator(x_train, y_train, args.batch_size_train)
    val_gen = list_generator(x_test, y_test, args.batch_size_val)
    model.fit_generator(generator=train_gen,
                        steps_per_epoch=int(y_train.shape[0] / args.batch_size_train),
                        epochs=args.epochs,
                        validation_data=val_gen,
                        validation_steps=int(y_test.shape[0] / args.batch_size_val),
